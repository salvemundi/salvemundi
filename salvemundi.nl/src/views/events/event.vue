<template>
  <b-container v-if="event" class="my-3">
    <b-row v-if="event.sections">
      ...
      <b-col cols="6">
        <img src="/introduction/69623406_1139359282931735_3484550277695012864_o.jpg"/>
      </b-col>
      <b-col cols="6">
        <img src="/introduction/70476475_1139360476264949_5210568686154285056_o.jpg"/>
      </b-col>
      <b-col></b-col>

      <b-col v-for="(section, index) in event.sections" :key="index" :cols="section.columns">
        <b-row>
          <b-col>
            <h5>{{ section.heading }}</h5>
          </b-col>

          <b-col v-for="(contentItem, index) in section.content" :key="index" :cols="contentItem.columns" class="p-0">
            <p v-if="contentItem.type === 'text'">{{ contentItem.content }}</p>
            <p v-if="contentItem.type === 'button'">
              <b-button variant="samu" :to="contentItem.to">{{ contentItem.content }}</b-button>
            </p>
            <img v-if="contentItem.type === 'image'" :src="contentItem.path"/>
          </b-col>
        </b-row>
      </b-col>

      <b-col cols="12" class="mt-2">
        <b-button :to="{ name: 'form.signup', params: { id: event.formId } }" variant="samu" v-if="event.hasSignupForm">
          {{$t('action')}}
        </b-button>
      </b-col>
    </b-row>
...
<i18n src="@/lang/events/event.json"></i18n>
